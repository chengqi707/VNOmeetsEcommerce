...
class MultiAgentAsyn:
    ...
    def __init__(self,
        ...
        self.model_path = os.path.join(model_path, '{}_agents'.format(self.num_agent+1), benchmark)
    ...
    def step(self):
        ...
        if self.training:
            ...
            self.obs_buffer_VNO[self.buffer_index] = self.observations[1]
            self.act_buffer_VNO[self.buffer_index] = np.array(actions[1])
            self.reward_buffer_VNO[self.buffer_index] = reward1  # ! here reward is a float variable, not an array
            self.next_obs_buffer_VNO[self.buffer_index] = new_observations[1]
    ...
    def train(self):
        ...
        loss /= self.num_agent
        """
        if self.episode_count < 15000 * self.episode_length:
        ...
        elif self.episode_count < 35000 * self.episode_length:
            loss[0] = 0
            loss[1] = self.agent_VNO.learn((obs_VNO[:, :, 0, :], actions_VNO[:, :, 0],
                                            rewards_VNO[:, :, 0], next_obs_VNO[:, :, 0, :]))
        elif self.episode_count < 45000 * self.episode_length:
            loss[0] = self.agent_NO.learn((obs_NO[:, :, :], actions_NO[:, :], rewards_NO[:, :], next_obs_NO[:, :, :]))
            loss[1] = 0
        else:
            loss[0] = self.agent_NO.learn((obs_NO[:, :, :], actions_NO[:, :], rewards_NO[:, :], next_obs_NO[:, :, :]))

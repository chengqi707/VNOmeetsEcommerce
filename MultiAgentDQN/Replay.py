...
class Replay:
    ...
    def __init__(self, num_VNO = 1, episode_length = 100, obs_size=4, capacity=10000) -> None:
        ...
        self.actions_NO = np.empty((self.capacity, self.episode_length), dtype=np.float32)
        ...
        self.actions_VNO = np.empty((self.capacity, self.episode_length, self.num_VNO), dtype=np.float32)
    ...
    def sample(self, batch_size, trace_length):
        ...
        actions_NO = np.empty((batch_size, trace_length), dtype=np.float32)
        ...
        actions_VNO = np.empty((batch_size, trace_length, self.num_VNO), dtype=np.float32)

...
class SingleVnoAgent:
    ...
    def learn(self, batch):
        ...
        actions = torch.FloatTensor(act_batch).flatten()
        rewards = torch.FloatTensor(reward_batch).flatten()

        loss = 0.0

        predicted_qvalues, _ = self.model(observations,
                                          batch_size=self.batch_size)  # ! shape = (batch, sequence, action)
